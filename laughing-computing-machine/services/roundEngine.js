...
async function tickAviator(game) {
  ...
  if (!openExists.rows[0]) {
    ...
    if (stale.rows[0]) {
      try {
        ...
        console.error('Aviator startNew failed:', e.message);
      }
    }
  }
  ...
  if (flying.rows[0]) {
    ...
    if (elapsed >= requiredElapsed || mult >= crash - 0.01) {
      try {
        ...
        console.error('Aviator declare failed:', e.message);
      }
    }
    return;
  }
  ...
  await pool.query(
    `UPDATE game_rounds SET closes_at = started_at + ($1 || ' seconds')::interval
     WHERE id = $2 AND status = 'open'
       AND closes_at > started_at + (($1 + 2) || ' seconds')::interval`,
    [betSeconds, roundId]
  );
...
async function tickGame(game) {
  ...
  const interval = Math.max(15, Number(game.auto_result_interval) || 60);
  ...
  if (seconds_left != null && seconds_left <= 0) {
    ...
    try {
      ...
      console.error(`Round declare failed for ${game.game_id}:`, e.message);
    }
  }
}
...
async function tick() {
  try {
    ...
    console.error('Round engine tick error:', e.message);
  }
}
