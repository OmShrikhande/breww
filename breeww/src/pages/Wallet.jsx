...
const WalletPage = () => {
  ...
  return (
    <div className="px-3 sm:px-4 py-4 space-y-4 select-none pb-24 animate-fadeIn">
      {/* Toast */}
      {toastMessage && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-[140] bg-[#1C0202] border-2 border-amber-400 text-amber-200 px-5 py-2.5 rounded-full text-xs font-black shadow-2xl flex items-center gap-2 max-w-[90%] text-center animate-bounce">
          <CheckCircle2 size={16} className="text-amber-400" /> {toastMessage}
        </div>
      )}

      {/* Balance Hero Card */}
      <div className="rounded-3xl border-2 border-amber-500/40 bg-gradient-to-br from-[#8B0000] via-[#450505] to-[#180202] p-5 sm:p-6 relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 p-4 opacity-15 pointer-events-none">
          <Coins size={90} className="text-amber-400" />
        </div>
        <span className="text-[10px] font-black uppercase tracking-widest text-amber-200/70 block mb-1">
          Total Wallet Balance
        </span>
        <p className="text-3xl sm:text-4xl font-black text-amber-300 font-mono tracking-tight tabular-nums drop-shadow">
          {formatINR(balance)}
        </p>
      </div>

      {/* Recharge / Cashout Tabs */}
      <div className="flex gap-1.5 p-1 rounded-2xl bg-[#180202]/95 border border-amber-500/30 shadow-lg">
        {[
      ...
      {tab === 'recharge' && (
        <div className="space-y-2">
          <span className="text-[10px] font-black uppercase tracking-wider text-amber-300/70 px-1 block">
              ...
              <button
                ...
                className={`p-3 rounded-2xl border text-left transition-all cursor-pointer flex items-center justify-between ${
                  selectedChannel === ch.id
                    ? 'border-amber-400 bg-amber-500/20 shadow-[0_0_15px_rgba(255,215,0,0.3)]'
                    : 'border-amber-500/20 bg-[#1C0202]/95 text-white/70 hover:border-amber-400/40'
                }`}
              >
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-black/50 border border-amber-500/30 flex items-center justify-center text-amber-400">
                    <ch.icon size={16} />
                  </div>
                  <div>
                ...
                {ch.popular && (
                  <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded-full bg-red-600 text-white border border-white/40">
                    HOT
                  </span>
                )}
              </button>
      ...
      <div className="rounded-3xl border border-amber-500/30 bg-[#1C0202]/95 p-5 space-y-4 shadow-xl">
        <div>
          <span className="text-[10px] font-black uppercase tracking-wider text-amber-300/70 block mb-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-amber-500/30 bg-black/50 px-8 py-3.5 text-2xl font-black text-white text-center focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 tabular-nums"
            />
          </div>
        </div>
          ...
          {QUICK_AMOUNTS.map((val) => (
            <button
              ...
              className={`py-2 rounded-xl text-xs font-mono font-black border transition-all active:scale-95 cursor-pointer flex items-center justify-center ${
                Number(amount) === val
                  ? 'border-white/80 bg-gradient-to-r from-amber-400 to-yellow-500 text-red-950 shadow-md font-black'
                  : 'border-amber-500/20 bg-black/40 text-amber-200/80 hover:border-amber-400/40 hover:text-white'
              }`}
            >
      ...
      <div className="rounded-3xl border border-amber-500/30 bg-[#1C0202]/95 overflow-hidden shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-amber-500/20 bg-black/40">
          ...
          <span className="text-[10px] text-amber-300/60">{txs.length} records</span>
        </div>
      ...
      {showQrModal && (
        <div className="fixed inset-0 z-[130] bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="w-full max-w-sm rounded-3xl bg-[#1C0202] border-2 border-amber-500/40 p-5 sm:p-6 shadow-2xl text-white space-y-4 my-auto animate-fadeIn">
            {/* Modal Header */}
            ...
            <div className="text-center space-y-3 bg-black/50 p-3.5 rounded-2xl border border-white/5">
              {/* QR Code Container */}
              <div className="my-1 flex items-center justify-center">
                {qrConfig.qrImageUrl ? (
                  <div className="relative mx-auto bg-white p-3 rounded-2xl w-48 h-48 flex items-center justify-center shadow-inner border-2 border-amber-400">
                    <img src={qrConfig.qrImageUrl} alt="UPI QR Code" className="w-full h-full object-contain rounded-lg" />
                  </div>
                ) : (
                  <QRCodeView
                    value={upiDeepLink}
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/10 text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-black text-amber-400 text-sm">₹{amount}</span>
                    <button
                      type="button"
                      onClick={handleCopyAmount}
                      className="p-1 rounded bg-amber-500/20 text-amber-300 hover:bg-amber-500/30 transition-colors cursor-pointer"
                      title="Copy Amount"
                    >
              ...
              <div className="grid grid-cols-3 gap-1.5 pt-1">
                <a
                  href={upiDeepLink}
                  className="py-2 px-1 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[10px] uppercase flex items-center justify-center gap-1 shadow"
                >
                  PhonePe
                </a>
                <a
                  href={upiDeepLink}
                  className="py-2 px-1 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-black text-[10px] uppercase flex items-center justify-center gap-1 shadow"
                >
                  Paytm
                </a>
                <a
                  href={upiDeepLink}
                  className="py-2 px-1 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-[10px] uppercase flex items-center justify-center gap-1 shadow"
                >
                  GPay
                </a>
              </div>
            ...
            <form onSubmit={handleUtrSubmit} className="space-y-2.5">
              ...
              <input
                ...
                className="w-full h-11 px-3.5 rounded-xl bg-black/60 border border-amber-500/30 text-white font-mono font-black text-center text-sm focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 tracking-widest"
              />
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-green-600 text-white font-black text-xs uppercase tracking-wider shadow-lg hover:brightness-110 active:scale-95 transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed border border-white/30"
              >
